...
is
   ...
   T4 : Token_Array (1 .. 0) := (others => (Offset => 0, Length => 0, Next_C => Character'Val (65)));
   Error : Boolean;
   B     : Byte_Array (1 .. 100) :=
...
--     end loop;
   ...
   if not Error then
      Put ("No error reported T1. Got this many bytes: ");
   ...
   if not Error then
      Put ("No error reported T2. Got this many bytes: ");
      Put (BLen);  -- print the length of the output array
      New_Line;
      -- If Output_Length <= Output_Array'Length, then the output array
      if BLen <= B'Length then
   ...
   if not Error then
      Put ("No error reported T3. Got this many bytes: ");
      Put (BLen);  -- print the length of the output array
      New_Line;
      -- If Output_Length <= Output_Array'Length, then the output array
      if BLen <= B'Length then
   ...
   if not Error then
      Put ("No error reported T4. Got this many bytes: ");
      Put (BLen);  -- print the length of the output array
      New_Line;
      -- If Output_Length <= Output_Array'Length, then the output array
      if BLen <= B'Length then
   ...
   if Is_Valid (T3) then -- Check the validity of the input array
      Put("Valid and run the Deode_Fast");
      Decode_Fast (T3, B, BLen);
      Put ("Got this many: ");
      Put (BLen);
      New_Line;
